# The current code replaces all missing values for op_any as ==1
# Code starts
...
copd$op_any[copd$op_any==7|copd$op_any==9]<-1 # re-coding missing data
...
# removing NA.
copd<-copd[!is.na(copd$depressive)==T,] # keeping observations without missing data for main exposure and outcome. 

table(copd$depressive, copd$op_any, useNA = 'always', deparse.level = 2)

write.csv(copd, "copd_na_equals_one.csv")
